...
class TrainingModel(InferenceModel):
    ...
    def save_nets(self, epoch, batch_id, latest=False, training_phase='main'):
        print(type(self.opt))
        block_path0 = self.opt.block_path0
        block_path1 = self.opt.block_path1
        block_path2 = self.opt.block_path2
        if not latest:
        ...
        torch.save(self.net.tuning_blocks.tuning_block0.cpu().state_dict(), block_path0)
        print('saved %s' % (block_path0))
        torch.save(self.net.tuning_blocks.tuning_block1.cpu().state_dict(), block_path1)
        print('saved %s' % (block_path1))
        torch.save(self.net.tuning_blocks.tuning_block2.cpu().state_dict(), block_path2)
        print('saved %s' % (block_path2))
    ...
    def load_pre_trained_main(self):
        if os.path.exists(self.opt.pre_trained_main_model):
            ...
            raise Exception('!!! no pre_trained_main_model in %s' % self.opt.pre_trained_main_model)
